...
class PersonalTaskController extends Controller
{
    public function index(){
        ...
        $sn = $user->idno.'_'.$user->name;
        // dd($sn);
        // $taskdata = DB::table('tasks')
        // ->where('tasks.task_assign_to','LIKE',"%{$sn}%")
        // ->join('projects','tasks.task_project_id','=','projects.id')
        // ->join('userprojects','projects.id','=','userprojects.project_id')
        // ->join('users','.users.id','=','userprojects.user_id')
       
        // ->orderBy('tasks.task_created_at','desc')
        // ->select('tasks.*','projects.project_name','projects.project_created_by','users.name','users.idno')
        // ->get();

        $todo_taskdata = DB::table('tasks')
        ->where('tasks.task_assign_to','LIKE',"%{$sn}%")
        ->where('tasks.task_status','=','todo')
        ->join('projects','tasks.task_project_id','=','projects.id')
        ->orderBy('tasks.task_created_at','desc')
        ->select('tasks.*','projects.project_name')
        ->get();
        $todo_taskdata_count = count($todo_taskdata);

        $complete_taskdata = DB::table('tasks')
        ->where('tasks.task_assign_to','LIKE',"%{$sn}%")
        ->where('tasks.task_status','=','complete')
        ->join('projects','tasks.task_project_id','=','projects.id')
        ->orderBy('tasks.task_created_at','desc')
        ->select('tasks.*','projects.project_name')
        ->get();

        $complete_taskdata_count = count($complete_taskdata);

        $inprogress_taskdata = DB::table('tasks')
        ->where('tasks.task_assign_to','LIKE',"%{$sn}%")
        ->where('tasks.task_status','=','inprogress')
        ->join('projects','tasks.task_project_id','=','projects.id')
        ->orderBy('tasks.task_created_at','desc')
        ->select('tasks.*','projects.project_name')
        ->get();
        $inprogress_taskdata_count = count($inprogress_taskdata);
        

        // dd($taskdata);

        return view('personal.tasks.personal-tasks',compact('todo_taskdata','complete_taskdata','inprogress_taskdata','todo_taskdata_count','complete_taskdata_count','inprogress_taskdata_count','user')); 
